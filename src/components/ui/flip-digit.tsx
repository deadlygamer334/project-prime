...
const FlipDigit = React.memo(({ value, label, isRetro, fontClass, isDark }: FlipDigitProps) => {
    ...
    if (!isRetro) {
        ...
        return (
            <div className="flex flex-col items-center select-text">
                <div className={`text-[15vw] max-md:landscape:text-[8vw] md:text-8xl leading-none font-bold ${fontClass} drop-shadow-2xl ${isDark ? "text-white" : "text-foreground"}`}>
                    {formatTimeDigit(value)}
                </div>
                <span className={`text-xl max-md:landscape:hidden md:text-2xl font-bold uppercase tracking-[0.2em] mt-2 opacity-50 ${isDark ? "text-white" : "text-foreground"}`}>{label}</span>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col items-center select-text">
            ...
            <span className={`text-xl max-md:landscape:hidden md:text-2xl font-bold uppercase tracking-[0.2em] mt-4 opacity-50 ${isDark ? 'text-white' : 'text-foreground'}`}>{label}</span>
        </div>
    );
});
