...
function MinimalPomodoro({ onComplete }: MinimalPomodoroProps) {
    ...
    const handleTimerCompleteWrapper = useCallback((m: TimerMode, duration: number, subject: Subject) => {
        ...
        // Send notification using new notification system - Fire and forget prevents blocking DB updates
        if (settings.notificationsEnabled) {
            sendTimerNotification(m, duration, subject).catch(console.error);
        }
    ...
    const togglePiP = useCallback(async () => {
        ...
        if (!("documentPictureInPicture" in window)) {
            showAlert("Not Supported", "Picture-in-Picture API is not supported in this browser.");
            return;
        }
    ...
    const handleTouchStart = (e: React.TouchEvent) => {
        ...
        if (e.touches.length === 1) {
            ...
            // Still track touchStartY for relative movement if needed, 
            // but pinch-to-zoom is the primary 2-finger gesture now.
    ...
    const handleTouchMove = (e: React.TouchEvent) => {
        ...
        if (e.touches.length === 1) {
            ...
            const nextScale = Math.min(2, Math.max(0.4, initialScale.current * scaleFactor));
            setScale(nextScale);
        }
    };
        ...
        <div className={`flex flex-col items-center justify-center ${isPiP ? "w-full h-full p-4 overflow-hidden" : ""}`}>
            ...
            {isPiP && (
                <div className="flex flex-col items-center mb-6 gap-1">
                    <div className={`px-3 py-1 rounded-full text-[10px] font-bold tracking-[0.2em] uppercase border ${isDark ? "bg-white/10 border-white/10 text-white/70" : "bg-black/5 border-black/10 text-black/70"
                        }`}>
            ...
            <div className={`relative flex items-end gap-2 max-md:landscape:gap-1 md:gap-4 ${isPiP ? "p-4 rounded-3xl border bg-card/20 border-border" : "p-4 max-md:landscape:p-1 md:p-8 rounded-[3rem] border backdrop-blur-xl shadow-2xl"} ${!isPiP && (isDark
                ...
                {[
                    ...
                    <div key={item.unit} className="flex items-end gap-2 md:gap-4">
                        {i > 0 && <span className={`text-3xl max-md:landscape:text-xl md:text-6xl ${fontClass} font-light mb-4 max-md:landscape:mb-0 ${isDark ? "text-white/10" : "text-black/10"}`}>:</span>}
                        <div className="flex flex-col items-center">
                            {editingUnit === item.unit ? (
                                <input
                                    ...
                                    className={`w-[1.5em] bg-transparent text-center ${fontClass} text-5xl md:text-8xl font-bold leading-none outline-none border-b-2 ${isDark ? "text-white" : "text-black"}`}
                                    style={{ borderColor: "var(--color-button)" }}
                                />
                            ) : (
                                <div className="flex flex-col items-center gap-2">
                                    {!(isFocusStarted && mode === "FOCUS") && mode !== "STOPWATCH" && !isPiP && (
                                        <button
                                            type="button"
                                            onClick={() => handleAdjust(item.unit as any, 1)}
                                            className={`transition-all hover:-translate-y-1 max-md:landscape:hidden ${isFocusStarted ? "text-[var(--color-button)]" : ""} ${isDark
                                    ...
                                    <motion.span
                                        onClick={() => mode !== "STOPWATCH" && !isPiP && handleUnitClick(item.unit as any, item.val)}
            ...
            {/* Controls in PiP */}
            {isPiP && (
                <div className="flex items-center gap-6 mt-8 z-10 scale-90">
                    <button
                        onClick={handleStart}
                        className={`group relative flex items-center justify-center w-14 h-14 rounded-full border transition-all ${isActive
    ...
    return (
        <div className="grid grid-cols-1 max-md:landscape:grid-cols-2 items-center justify-center w-full max-w-2xl max-md:landscape:max-w-4xl mx-auto py-6 max-md:landscape:py-2 px-6 gap-6 max-md:landscape:gap-4 relative">
            ...
            {(mode === "FOCUS" || mode === "STOPWATCH") && (
                <div className="mb-6 max-md:landscape:mb-0 w-full max-w-xs relative z-20 order-2 max-md:landscape:order-3 justify-self-center max-md:landscape:justify-self-start">
                    <CustomSelect
